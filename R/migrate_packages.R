#' Migrate packages between R installations
#'
#' @param source_path Rscript path of the source installation
#' @param target_path Rscript path of the target installation
#' @param packages Optional character vector of packages to migrate. If NULL, migrates all non-base missing/outdated packages.
#' @param dry_run Logical. If TRUE, return plan without installing.
#' @param upgrade Logical. Passed to pak
#' @param ... Extra arguments
#' @return A list with migration results
#' @export
migrate_packages <- function(source_path, target_path, packages = NULL, dry_run = FALSE, upgrade = FALSE, ...) {
  start_time <- Sys.time()
  ...
  src_pkgs <- list_packages(rscript_path = source_path, format = "data.table")
  tgt_pkgs <- list_packages(rscript_path = target_path, format = "data.table")
  
  comp <- compare_libraries(src_pkgs, tgt_pkgs)
  ...
  tgt_pkgs_after <- list_packages(rscript_path = target_path, format = "data.table")

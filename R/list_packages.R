...
#' @return data.table
#' @export
list_packages <- function(rscript_path = NULL, lib_path = NULL, format = c("data.table", "data.frame"), timeout_sec = 30L) {
  ...
  if (!fs::file_exists(rscript_path)) {
    cli::cli_abort("Rscript not found at {.path {rscript_path}}", class = "packport_rscript_not_found")
  }
  
  script_file <- fs::path_temp("packport_list_packages.R")
...
')
  ...
  if (inherits(res, "error")) {
    cli::cli_abort("Failed to execute subprocess", parent = res, class = "packport_subprocess_error")
  }
  ...
  if (res$status != 0) {
    ...
    cli::cli_abort("Subprocess exited with status {res$status}", class = "packport_subprocess_error")
  }
  ...
  if (grepl("^CSV_START", out_text)) {
    ...
    if (inherits(parsed, "error")) {
      cli::cli_abort("Failed to parse JSON output", parent = parsed, class = "packport_json_parse_error")
    }
  }

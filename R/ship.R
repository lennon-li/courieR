...
#'
#' Compares the package libraries of two R installations and installs missing
#' or outdated packages into the target using
#' [pak::pkg_install()] running under the target R executable.
#'
...
#'   during the pak subprocess, including a warning if first-time metadata
#'   loading takes 1–2 minutes. Useful for surfacing progress in a UI.
#' @param ... Reserved for future arguments.
#' @return A named list with the following elements:
#'   \describe{
#'     \item{`plan`}{`data.table` of planned actions with columns `package`,
#'       `action` (`"install"` or `"upgrade"`), `version.x` (source version),
...
#'       `package`, `status` (`"success"` or `"error"`), and `message`.}
...
#' }
#' @export
ship <- function(source_path, target_path, packages = NULL, dry_run = FALSE, upgrade = FALSE, log_callback = NULL, ...) {
  start_time <- Sys.time()

...
The Sync tab mirrors the `ship()` CLI workflow. Unmatched and outdated packages
are shown first in the comparison table. Before installing packages, the
confirmation dialog shows an approximate time range based on the number of
packages in the sync plan. During sync, the dashboard shows progress details and
a log of the direction, package list, completion state, and any failures. After
sync finishes, courieR rescans both selected installations and refreshes the
...
- The source R installation does not need extra packages; the target R
  installation needs `pak` available because installation runs under the target R

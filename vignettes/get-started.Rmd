...
<!-- NOTE: this screenshot predates the source/target rename (it shows the old
     "first/second installation" pickers and the removed sync-direction selector).
     A human should refresh figures/sync-tab.png to match the current UI. -->
```{r sync-screenshot, echo=FALSE, out.width="100%", fig.cap="The Bulk Dispatch tab: pick a source and a target R installation, compare their libraries, and ship packages from source into target. The log pane on the right shows real-time progress."}
img <- "figures/sync-tab.png"
if (file.exists(img)) knitr::include_graphics(img)
```

**Workflow:**

1. The dashboard scans all R installations on the machine automatically — no configuration needed.
2. Select a **source** and a **target** installation from the sidebar dropdowns. The target list is constrained to the same-or-newer R version than the source, since an older R can't reliably hold packages built for a newer one.
3. Click **Compare** — a summary strip shows counts of identical, missing, and version-mismatched packages. The table lists differences first.
4. Click **Ship** to install or upgrade the source's packages into the target. To mirror two same-version installs, run the transfer again with source and target swapped.
5. A confirmation dialog shows the package count and an estimated time. Confirm to start.
6. The log pane shows real-time progress: each package, its pak spec, success or failure, and a post-sync comparison refresh.

The first sync on a machine can take 1–2 minutes while pak builds its metadata cache. Subsequent syncs are faster.

The **Browse** and **Manifest** tabs expose `manifest()` output and let you inspect any detected R installation's full package list.

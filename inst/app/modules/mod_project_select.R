mod_project_select_ui <- function(id) {
  ns <- NS(id)
  tagList(
    bslib::card(
      bslib::card_header("Project Selection"),
      shiny::textInput(ns("path"), "Project Path", value = "."),
      shiny::actionButton(ns("scan"), "Scan Project", class = "btn-primary")
    ),
    uiOutput(ns("scan_results"))
  )
}

mod_project_select_server <- function(id, global_project_path) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$scan, {
      ...
      path <- input$path
      if (dir.exists(path)) {
        res <- packport::detect_project_type(path)
        scan_res(res)
